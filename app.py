...
def form():
    ...
    if request.method == "POST":
        ...
        if not fullname.strip():
            ...
            return render_template("form.html", fullname = None, error = error)
...
def calc():
    ...
    if request.method =="POST":
        ...
        if not a or not b:
            ...
            return render_template("form.html", result = None, error = error, a=a, b=b, operator = operator)

        if operator == "/" and b=="0":
            error = "Nie wolno dzielic przez 0"
